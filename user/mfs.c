...
void ICACHE_FLASH_ATTR FindMPFS()
{
	...
	if( strncmp( "MPFSMPFS", (char*)mfs_check, 8 ) == 0 ) { mfs_at = MFS_START; goto done; }
	...
	if( strncmp( "MPFSMPFS", (char*)mfs_check, 8 ) == 0 ) { mfs_at = MFS_ALTERNATIVE_START; goto done; }

	os_printf( "MFS Not found at alternative address (%08x%08x).\n", mfs_check[0], mfs_check[1] );

done:
	//os_printf("flahsector[%s",(char*)mfs_check);
	//os_printf("myfile with luck [%s]\n",(char*)mfs_check[0]+0x100);
