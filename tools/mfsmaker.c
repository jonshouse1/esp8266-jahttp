...
// Jons notes:
// The ESP12 seems to have 4Megabytes of flash.   
// Image starting at 0x100000 ending 0x400000 = 0x300000 or 3 Megabytes big

// Uncomment one of these
//#define FS_SIZE		8							// 1MB
//#define FS_SIZE		16							// 16=2MB
#define FS_SIZE		24							// 24=3MB
...
unsigned char mfsdata[131072*FS_SIZE];
...
int main( int argc, char ** argv )
{
	...
	// Tell the project how many files are in the memory file system
	printf("encoded %d files\n",filecount);
	FILE * x = fopen( "mfs_filecount.h", "w" );
	if( !x || ferror( f ) )
	{
		fprintf( stderr, "Error: cannot open mfs_filecount.h for writing.\n" );
	}
	fprintf(x,"#define MFS_FILECOUNT\t%d\n",filecount);
	fclose(x);
